...
How to run Chat Room on your own machine — natively with Node/npm, with a single
...
docker run --rm -p 824:824 Chat Room
```
...
```bash
docker run -d --name Chat Room -p 824:824 Chat Room   # start detached
docker logs -f Chat Room                          # follow logs
docker stop Chat Room && docker rm Chat Room           # stop & remove
```
